...
| Jul 2026 | Antigravity | **Dashboard Dinâmico e Prova de Conceito Visão:** (1) Corrigido o bug do card "Sono" vazio no Next.js (`page.tsx`) migrando a renderização de chaves fixas para `Object.entries` dinâmico e flexibilizando os tipos no `manolo.ts`, tornando a UI imune a chaves aleatórias do LLM. (2) Concluída a Fase 1 da Prova de Conceito de **Visão Computacional** com a criação do `scripts/test_vision.py` rodando OpenAI Vision com Structured Outputs (`LLMChecklistResponse`). (3) Criado `future_plan.md` detalhando a migração do LangGraph para `MessagesState` para dar suporte a memória conversacional em contextos de imagens ambíguas. |
| Jul 2026 | Antigravity | **Desacoplamento de Terapias e Componentes Booleanos:** (1) Substituídos todos os componentes `Toggle` por um novo componente `SelectBoolean` (`Não informado / Sim / Não`) nos checklists para evitar ambiguidade estatística (vazio vs falso). (2) Lógica de Terapias foi completamente removida dos Checklists Diários. Criado novo módulo full-stack de Terapias independentes, com novas rotas de API exclusivas (`GET, POST, PATCH, DELETE /api/terapias`) e uma nova página dedicada no menu lateral do Web App, com cards responsivos. |
| Jul 2026 | Antigravity | **Sincronização UI/DB e Refatoração de Roteamento de Modelos:** (1) Centralização de 12 chaves de modelos independentes (`LLM_MODEL_*`) no arquivo `.env` para facilitar A/B testing e migração gradual para a família GPT-5. (2) Investigação de dependência temporal e correção de paridade no formulário web (React) para inclusão das caixas `banho` na seção de Higiene e renderização de todos os campos da tabela `checklist_movimento` (Atividades, Caiu muito, Buscou colo), garantindo que dados estruturados via áudio possam ser editados no painel. |
| Jul 2026 | Antigravity | **Validação de Modelos de Reasoning (GPT-5/Claude Sonnet) na Extração:** (1) Criado `scripts/test_extraction.py` isolado para testar extração (Structured Outputs) em mensagens complexas do WhatsApp sem depender do banco de dados. (2) Confirmado o poder semântico do GPT-5 em classificar dados subjetivos ("cochilo", "faz de conta") nativamente, sem necessidade de regras clínicas pesadas ou "prompt hacking". (3) Refatorado `agent.py` injetando `temperature=0` condicionalmente (apenas para GPT-4), prevenindo `Error 400 Bad Request` com modelos de reasoning. (4) Validado que as rotas FastAPI (webhook) já utilizam `BackgroundTasks`, garantindo desacoplamento seguro contra timeouts da Meta devido à alta latência do GPT-5. |
